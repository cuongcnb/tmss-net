...
@Component({
  ...
  selector: 'dong-son-progress-by-car',
  templateUrl: './dong-son-progress-by-car.component.html',
  styleUrls: ['./dong-son-progress-by-car.component.scss']
})
export class DongSonProgressByCarComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  functionCode = TMSSTabs.dongsonProgressByCar;
  ...
  ngOnDestroy() {
    this.progressTrackingService.destroyComponent(this.car);
    this.destroy();
  ...
  getData(isClearInterval?) {
    ...
    this.roWshopApi.searchRoWshopDsVehicle(this.form.value).subscribe(res => {
      this.temp_list = res.data.concat(res.planData);
      this.stateCount = res.stateCount;
      this.shops = res.vehicles;
  ...
  getCurrentWorkTime(isClearInterval?) {
    this.dlrConfigApi.getCurrentWorkTime(this.form.value, RoType.DS).subscribe(workTimes => {
      if (workTimes) {
        ...
        this.progressTrackingService.generateBreaktime(workTimes, this.elem.nativeElement.querySelectorAll('.car-td'), this.car);
        if (isClearInterval) {
          clearInterval(this.interval);
        }
  ...
  searchByRegisterNo(keyword) {
    ...
    const elems = document.querySelectorAll('.car-register-no-progress');
    const focusVehicleElem = document.querySelectorAll('.car-focus-vehicle');
    if (focusVehicleElem) {
      focusVehicleElem.forEach(it => it.classList.remove('car-focus-vehicle'));
    }
    ...
    if (elems && elems.length) {
      ...
      if (this.registerNoSearch.length) {
        ...
        for (let i = 0; i < this.registerNoSearch.length; i++) {
          ...
          this.registerNoSearch[i].parentElement.classList.add('car-focus-vehicle');
        }
    ...
    if (none_result) this.swalAlert.openWarningToast("Không tìm thấy khoang này!")
  }

  showCompleteCar() {
    const elems = document.querySelectorAll('.car-tracking-progress');
    if (elems && elems.length) {
      elems.forEach(elem => {
  ...
  search(isLoading?) {
    ...
    // this.roWshopActApi.searchRoWshopDs(this.form.value).subscribe(res => {
      this.loadingService.setDisplay(false);
      ...
      if (list && list.length) {
        const result = list.map(item => {
          return {
            ...item, ...{index: this.shops.findIndex(shop => shop.idTmp === item.waitProcessIdTmp)}
          };
        });
        ...
        this.progressTrackingService.generatePlan(result, this.workTimes, this.shops, this.car, this.listDsDefault, true, true);
        setTimeout(() => {
    ...
    if (todayWorktime) {
      ...
      setTimeout(() => {
        if (new Date().getHours() >= start && new Date().getHours() < end) {
          ...
          this.delayVehicles = this.progressTrackingService.autoIncreaseActualJobWidth(this.list, this.car).totalActualLate;
          this.delayPlanVehicles = this.progressTrackingService.autoIncreaseActualJobWidth(this.list, this.car).totalPlanLate;
        }
      }, 1000);
  ...
  initPositionData() {
    ...
    const areas = document.querySelectorAll('.car-progress-area');
    const col132 = document.querySelector('.col132');
    if (!areas || !areas.length || !col132) {
      return;
    }
    this.minX = areas[0].getBoundingClientRect().left;
    this.minY = document.querySelector('.car-tbody').getBoundingClientRect().top;
