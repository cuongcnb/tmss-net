...
@Component({
  ...
  styleUrls: ['./sales-order-modal.component.scss']
})
export class SalesOrderModalComponent implements OnInit {
  ...
  uploadFile() {
    ...
    this.importService.loadDlrSalesOrder(this.uploader);
  ...
  saveFile() {
    this.loadingService.setDisplay(true);
    this.importService.saveFileSalesOrder(this.importDate).subscribe(res => {
      this.loadingService.setDisplay(false);
