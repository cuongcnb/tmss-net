...
@Component({
  selector: 'sales-target-modal',
  templateUrl: './sales-target-modal.component.html',
  styleUrls: ['./sales-target-modal.component.scss']
})
export class SalesTargetModalComponent implements OnInit {
  ...
  uploadFile() {
    ...
    this.importService.loadTmvSalesTarget(this.uploader);
  ...
  saveFile() {
    this.loadingService.setDisplay(true);
    this.importService.saveFileSalesTarget(this.importDate).subscribe(res => {
      this.loadingService.setDisplay(false);
