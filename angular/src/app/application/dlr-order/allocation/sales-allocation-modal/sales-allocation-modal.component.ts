...
@Component({
  selector: 'sales-allocation-modal',
  templateUrl: './sales-allocation-modal.component.html',
  styleUrls: ['./sales-allocation-modal.component.scss']
})
export class SalesAllocationModalComponent implements OnInit {
  ...
  uploadFile() {
    ...
    this.importService.loadTmvSalesAllocation(this.uploader);
  ...
  saveFile() {
    this.loadingService.setDisplay(true);
    this.importService.saveFileSalesAllocation(this.importDate).subscribe(res => {
      this.loadingService.setDisplay(false);
