...
export class AddUpdateBPModalComponent extends AppComponentBase implements OnInit {
  ...
  save() {
    ...
    if (this.type === 'job') {
      ...
      apiCall = this.selectedJob
        ? this.srvDRcJobsModelsApi.updateRcJobModel(value, 'Y')
        : this.srvDRcJobsModelsApi.insertRcJobModel(value, 'Y');
    } else {
      const partList = this.gridTableService.getAllData(this.partGridParams);
  ...
  private buildForm() {
    this.form = this.formBuilder.group({
      ...
      price: [{value: undefined, disabled: this.type === 'part'},
        Validators.compose([GlobalValidator.required, GlobalValidator.floatNumberFormat, GlobalValidator.maxLength(53)])],
      jobgroup: [{value: 5, disabled: true}, GlobalValidator.required],
      status: ['Y'],
      dlrId: [{
        value: this.currentUser.dealerId,
        disabled: this.selectedJob || this.type === 'part'
      }, GlobalValidator.required],
      cfId: [{
        value: undefined,
  ...
  addJob(data) {
    ...
    data.dlrId = this.form.getRawValue().dlrId;
