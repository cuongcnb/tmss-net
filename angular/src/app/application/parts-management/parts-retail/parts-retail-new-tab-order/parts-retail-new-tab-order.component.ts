...
@Component({
  ...
  selector: 'parts-retail-new-tab-order',
  templateUrl: './parts-retail-new-tab-order.component.html',
  styleUrls: ['./parts-retail-new-tab-order.component.scss']
})
export class PartsRetailNewTabOrderComponent implements OnInit, OnChanges {
  ...
  ngOnChanges() {
    if (this.selectDataPartsRetail) {
      this.selectedOrder = this.selectDataPartsRetail.selectedOrder ? this.selectDataPartsRetail.selectedOrder : undefined;
      this.orderDetailData = this.selectDataPartsRetail.orderDetailData ? this.selectDataPartsRetail.orderDetailData : undefined;
      this.patchData();
    }
  }
  ...
  patchData() {
    ...
    if (this.params) {
      this.params.api.setRowData(this.orderDetailData ? this.orderDetailData.parts : []);
    ...
    if (+params.data.qty < 0 && +params.data.dxQty) {
      ...
      if (dupplicatedParts.length) {
        ...
        if (totalQty - totaldxQty < 0) {
          this.toastService.openWarningToast(`SL Cần không hợp lệ, kiểm tra lại phụ tùng ${dupplicatedParts[0].partsCode}`);
          return false;
        }
      }
    }
  ...
  viewOrder() {
    this.getDisplayedData();
    const dataGroup = groupBy(this.displayedData, 'partsId');
    const listPartId = [];
    this.displayedData.forEach(it => listPartId.push(it.partsId));
    const listPartIdNotDuplicate = new Set(listPartId);
    for (const it of listPartIdNotDuplicate) {
      let qty = 0;
      const data = dataGroup[`${it}`];
      const qtyReceived = data[data.length - 1].dxQty;
      for (const el of dataGroup[`${it}`]) {
        qty += el.qty;
      }
      if (qtyReceived > qty) {
        if (qtyReceived === 0) {
          this.toastService.openWarningToast(`Tổng số lượng của mã phụ tùng ${dataGroup[`${it}`][0].partsCode} đang nhỏ hơn 0. Vui lòng kiểm tra lại`);
          return;
        } else {
          this.toastService.openWarningToast(`Chú ý liên hệ phòng phụ tùng để xử lý mã phụ tùng ${dataGroup[`${it}`][0].partsCode} trước khi điều trỉnh s.lượng!`);
          return;
        }
      }
    }
