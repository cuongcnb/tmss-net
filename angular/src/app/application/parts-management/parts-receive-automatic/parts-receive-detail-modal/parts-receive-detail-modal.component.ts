...
@Component({
  ...
  selector: 'parts-receive-detail-modal',
  templateUrl: './parts-receive-detail-modal.component.html',
  styleUrls: ['./parts-receive-detail-modal.component.scss']
})
export class PartsReceiveDetailModalComponent implements OnInit {
  ...
  getPartsByInvoice() {
    ...
    this.partsReceiveAutomaticApi.getPartByInvoice(this.selectedInvoiceNode.data.id).subscribe((val: { parts: PartsReceiveModel[] }) => {
      if (val) {
  ...
  showDeliveryOrder() {
    ...
    if ((!receiveParts.length || receiveParts.length < girdData.length) && this.selectedInvoiceNode && this.selectedInvoiceNode.data.svourcher.indexOf('R') < 0) {
      this.swalAlertService.openWarningToast('Chỉ được nhận âm khi trả lại hàng');
      return;
    }
