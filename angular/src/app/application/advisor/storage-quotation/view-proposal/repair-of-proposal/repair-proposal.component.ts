...
@Component({
  ...
  templateUrl: './repair-proposal.component.html',
  styleUrls: ['./repair-proposal.component.scss'],
})
export class RepairProposalComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.roType && !this.isRefresh) {
      ...
      this.roType === '1'
        ? this.tabs.unshift({tab: 'dongson', name: 'Đồng Sơn'})
        : this.tabs.unshift({tab: 'repair', name: 'Sửa chữa chung'});
  ...
  private checkForm() {
    ...
    // Bảo hiểm
    this.proposalForm.get('isInr').valueChanges.subscribe(val => {
      if (val) {
