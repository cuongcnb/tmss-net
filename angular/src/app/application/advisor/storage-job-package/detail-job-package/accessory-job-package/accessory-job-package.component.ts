...
@Component({
  ...
  selector: 'accessory-job-package',
  templateUrl: './accessory-job-package.component.html',
  styleUrls: ['./accessory-job-package.component.scss'],
})
export class AccessoryJobPackageComponent implements OnInit, OnChanges {
    ...
    if (params.column.colId === 'qty') {
      this.params.api.forEachNode(rowNode => {
        if (params.rowIndex === rowNode.rowIndex) {
          ...
          rowNode.setDataValue('discount',
            (Number(params.data.qty) * params.data.sellPrice * Number(params.data.discountPercent || 0) / 100).toFixed(2)
          );
        }
      });
    }
  ...
  tabToNextCell(params) {
    if (params.previousCellDef && params.nextCellDef
      && params.previousCellDef.column.colId === 'discount' && params.nextCellDef.column.colId === 'discountPercent') {
    ...
    if (params.previousCellDef && params.nextCellDef
      && params.nextCellDef.column.colId === 'discount' && params.previousCellDef.column.colId === 'discountPercent') {
  ...
  cellDoubleClicked(params) {
    const col = params.colDef.field;
    if (this.state && this.state.toString() !== '1'
      && this.selectedNode && this.selectedNode.data.quotationprintVersion && col === 'partsCode') {
      return;
    }
    if (col === 'partsCode') {
  ...
  patchDiscount(data) {
    if (data.forAll) {
      ...
      this.selectedNode.setDataValue('discount',
        Number(data.discountPercent * this.doubleClickParams.data.qty * this.doubleClickParams.data.sellPrice / 100));
      this.calculateTotal();
    }
  }
    ...
    switch (col) {
      case 'qty':
        if (Number(data.qty) < 0) {
          this.swalAlertService.openWarningToast('Phải là số dương', 'Số không hợp lệ');
